...
const EmployeeAdd = () => {
  ...
  const validateForm = () => {
    ...
    if (!employeeId) {
      ...
      newErrors.employeeId = "Employee ID must not exceed 30 characters";
    } else if (!/^[A-Za-z0-9_-]+$/.test(employeeId)) {
      newErrors.employeeId =
        "Employee ID can contain only letters, numbers, _ and -";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await instance.post("/employee/create", payload, {
        withCredentials: true,
      });

      const createdEmployee = response?.data.result;
      ...
      if (status === 409) {
        ...
        if (message.includes("employee") && message.includes("id")) {
          setErrors((prev) => ({
            ...prev,
          ...
          setErrors((prev) => ({
            ...
            submit: backendMessage || "Employee ID or email already exists.",
          }));
        }
      } else {
        setErrors((prev) => ({
      ...
      dispatch(
        setEmployeeError(backendMessage || "Unable to create employee."),
      );
    } finally {
      setLoading(false);
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="border-b border-gray-200 px-5 py-5 md:px-7">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Enter the employee details carefully.
            </p>
          </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        min="1"
                        inputMode="numeric"
                        className={`w-full rounded-lg border py-3 pl-9 pr-4 text-sm text-gray-900 outline-none transition placeholder:text-gray-400 ${
                          errors.salary
                            ? "border-red-400 focus:ring-2 focus:ring-red-100"
                            : "border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                        }`}
